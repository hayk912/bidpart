...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Deleting field 'Invoice.due'
        db.delete_column('invoice_invoice', 'due')

        # Adding field 'Invoice.expiration'
        db.add_column('invoice_invoice', 'expiration',
                      self.gf('django.db.models.fields.DateTimeField')(default=datetime.datetime(2012, 10, 29, 0, 0)),
                      keep_default=False)

        # Adding unique constraint on 'Invoice', fields ['identifier']
        db.create_unique('invoice_invoice', ['identifier'])


    def backwards(self, orm):
        # Removing unique constraint on 'Invoice', fields ['identifier']
        db.delete_unique('invoice_invoice', ['identifier'])


        # User chose to not deal with backwards NULL issues for 'Invoice.due'
        raise RuntimeError("Cannot reverse this migration. 'Invoice.due' and its values cannot be restored.")
        # Deleting field 'Invoice.expiration'
        db.delete_column('invoice_invoice', 'expiration')
