...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for model in ['ads.businessdomain', 'ads.producttype', 'ads.productcategory']:
            for item in orm[model].objects.all():
                item.title_sv = item.title
                item.save()

        for field in orm['ads.Field'].objects.all():
            field.label_sv = field.label
            field.help_text_sv = field.help_text or ''
            field.save()

        for choice in orm['ads.FieldChoice'].objects.all():
            choice.name_sv = choice.name
            choice.save()

    def backwards(self, orm):
        for model in ['ads.businessdomain', 'ads.producttype', 'ads.productcategory']:
            for item in orm[model].objects.all():
                item.title = item.title_sv
                item.save()

        for field in orm['ads.Field'].objects.all():
            field.label_sv = field.label
            field.help_text = field.help_text_sv
            field.save()

        for choice in orm['ads.FieldChoice'].objects.all():
            choice.name = choice.name_sv
            choice.save()
