...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        # Normalize years.
        for value in orm['ads.Value'].objects.filter(field__pk=19):
            if not value.value.isdigit():
                value.value = ''
                value.save()

        # Normalize Horizontal / Vertical
        for value in orm['ads.Value'].objects.filter(field__pk=27, choice_value__pk=None):
            if value.value == u'Stående':
                value.choice_value_id = 19
            elif value.value == u'Liggande':
                value.choice_value_id = 20
            value.value = ''
            value.save()
